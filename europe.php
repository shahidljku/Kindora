...
try {
    $europe_destinations_result = $conn->query("
        ...
        WHERE d.type = 'europe' AND d.name != 'My Trip Planner'
        GROUP BY d.destination_id
        ORDER BY d.name
    ");
    $europe_destinations = $europe_destinations_result ? $europe_destinations_result->fetch_all(MYSQLI_ASSOC) : [];
} catch (Exception $e) {
    error_log("Error fetching Europe destinations: " . $e->getMessage());
    $europe_destinations = [];
}
...
<head>
    ...
    <title>Europe Destinations - Kindora</title>
    ...
    <meta name="description" content="Discover Europe with Kindora - From ancient history to modern cities">
</head>
<body>
    <!-- Background Video -->
    <div class="video-container">
        <video autoplay muted loop id="bg-video" preload="metadata">
            <source src="continents_videos/europe_intro.webm" type="video/webm">
            <source src="continents_videos/europe_intro.mp4" type="video/mp4">
        </video>
        ...
        <div id="nav2">
            <div class="inspire-wrapper">
                ...
                <div class="scroll-container1" id="inspireScroll">
                    ...
                    <a data-href="europe.php" class="active"><div class="image-wrapper1">
    ...
    <main>
        ...
        <div class="stats-section">
            <div class="container">
                <div class="stats-grid">
                    <div class="stat-item">
                        <span class="stat-number"><?= count($europe_destinations) ?></span>
                        <span class="stat-label">Destinations</span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-number"><?= array_sum(array_column($europe_destinations, 'booking_count')) ?></span>
                        <span class="stat-label">European Explorers</span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-number"><?= count($europe_destinations) > 0 ? number_format(array_sum(array_column($europe_destinations, 'avg_rating')) / count($europe_destinations), 1) : '0' ?></span>
        ...
        <div class="container">
            <div class="card-grid">
                <?php if (!empty($europe_destinations)): ?>
                    <?php foreach ($europe_destinations as $destination): ?>
                        <div class="video-card destination-card" 
                            ...
                            <div class="card-image">
                                <img src="<?= str_replace('\\\\', '/', htmlspecialchars($destination['image_url'])) ?>" 
                                     ...
                                     loading="lazy"
                                     onerror="this.src='continents/images/europe_images/default.avif'">
                                     
                                <?php if ($destination['booking_count'] > 8): ?>
                                    <div class="popular-badge">🏰 European Gem</div>
                                <?php endif; ?>
                            </div>
                ...
                <?php else: ?>
                    <div class="no-destinations">
                        <h3>European Adventures Coming Soon</h3>
                        <p>We're preparing incredible journeys across historic and modern Europe.</p>
                    </div>
                <?php endif; ?>
                ...
                <div class="video-card trip-planner-card" onclick="showTripPlanner()">
                    <div class="card-image">
                        <img src="continents/images/europe_images/trip_planner.avif" alt="My Trip Planner" loading="lazy">
                    </div>
                    <div class="card-overlay">
                        <h3>My European Journey Planner</h3>
                        <p>Plan your perfect European adventure through historic cities and cultural treasures.</p>
                        <div class="card-actions">
                            <button class="view-btn">📋 Plan European Tour</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
                ...
                .then(data => {
                    document.getElementById('modalContent').innerHTML = `
                        ...
                        <div class="modal-actions" style="margin-top: 20px;">
                            <a href="booking.php?destination=${data.destination_id}" class="book-btn">Book European Tour</a>
                        </div>
                    `;
        ...
        function showTripPlanner() {
            <?php if (isset($_SESSION['user_id'])): ?>
                window.location.href = 'trip_planner.php?continent=europe';
            <?php else: ?>
