...
// Fetch all Asia destinations from database
// NOTE: 'highlights' column does NOT exist - use available columns only
$asiaDestinations = array();
try {
    $asiaDestinations = KindoraDatabase::query(
        "SELECT destination_id, name, description, image_url, best_season, difficulty_level
         FROM destinations 
         WHERE type = 'asia' AND is_active = 1 
         ORDER BY name ASC"
    ) ?: array();
    
    // Log for debugging
    error_log("Asia destinations query result: " . count($asiaDestinations) . " records found");
} catch (Exception $e) {
    error_log("Asia destinations fetch error: " . $e->getMessage());
}

// Create an associative array for easy lookup by name
$destData = array();
foreach ($asiaDestinations as $dest) {
  ...
  <body>
    ...
    <div class="video-container">
      <video autoplay muted loop id="bg-video">
        <source src="/Kindora/continents videos/asia-intro.webm" type="video/webm" />
      </video>
    </div>

    <!-- Dynamically Loaded Destinations Grid -->
    <div class="card-grid">
      <?php if (!empty($asiaDestinations)): ?>
        <?php foreach ($asiaDestinations as $destination): ?>
            ...
            <div class="video-card" data-video="<?php echo htmlspecialchars($destination['name']); ?>.webm">
              ...
              <div class="card-content">
                ...
                <div class="card-actions">
                  ...
                  <button
                    class="info-btn"
                    onclick="handleBookNow(event, <?php echo htmlspecialchars($destination['destination_id']); ?>)"
                  >
                    ✈️ Book Now
                  </button>
                </div>
