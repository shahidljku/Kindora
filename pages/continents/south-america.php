...
// Fetch all Southamerica destinations from database
$southamericaDestinations = array();
try {
    $southamericaDestinations = KindoraDatabase::query(
        "SELECT destination_id, name, description, image_url, best_season, difficulty_level
         FROM destinations 
         WHERE type = 'southamerica' AND is_active = 1 
         ORDER BY name ASC"
    ) ?: array();

    // Log for debugging
    error_log("Southamerica destinations query result: " . count($southamericaDestinations) . " records found");
} catch (Exception $e) {
    error_log("Southamerica destinations fetch error: " . $e->getMessage());
}

// Create an associative array for easy lookup by name (if needed)
$destData = array();
foreach ($southamericaDestinations as $dest) {
...
<html lang="en">
  <head>
    ...
    <title>Southamerica - Kindora</title>
    ...
    <meta name="description" content="Explore the wild beauty of Southamerica with Kindora - Safari adventures and cultural experiences">
  </head>
  <body>
    <br><br><br>
    <!-- Background Video -->
    <div class="video-container">
      <video autoplay muted loop id="bg-video">
        <source src="/Kindora/continents videos/south_america_intro.webm" type="video/webm" />
      </video>
    </div>

    <!-- Dynamically Loaded Destinations Grid -->
    <div class="card-grid">
      <?php if (!empty($southamericaDestinations)): ?>
        <?php foreach ($southamericaDestinations as $destination): ?>
    ...
    <script>
      ...
      function displayItinerary() {
        ...
        let message = `✈️ Your Southamerican Adventure Plan:\n\n${tripList}\n\n🌟 ${itinerary.length} amazing destinations selected!`;
