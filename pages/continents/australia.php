...
// Fetch all Australia destinations from database
$australiaDestinations = array();
try {
    $australiaDestinations = KindoraDatabase::query(
        "SELECT destination_id, name, description, image_url, best_season, difficulty_level
         FROM destinations 
         WHERE type = 'australia' AND is_active = 1 
         ORDER BY name ASC"
    ) ?: array();

    // Log for debugging
    error_log("Australia destinations query result: " . count($australiaDestinations) . " records found");
} catch (Exception $e) {
    error_log("Australia destinations fetch error: " . $e->getMessage());
}

// Create an associative array for easy lookup by name (if needed)
$destData = array();
foreach ($australiaDestinations as $dest) {
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="Explore the wild beauty of Australia with Kindora - Safari adventures and cultural experiences">
  </head>
  <body>
    <br><br><br>
    <!-- Background Video -->
    <div class="video-container">
      <video autoplay muted loop id="bg-video">
        <source src="/Kindora/continents videos/australia_intro.webm" type="video/webm" />
      </video>
    </div>

    <!-- Dynamically Loaded Destinations Grid -->
    <div class="card-grid">
      <?php if (!empty($australiaDestinations)): ?>
        <?php foreach ($australiaDestinations as $destination): ?>
