...
// Fetch all Antarctica destinations from database
$antarcticaDestinations = array();
try {
    $antarcticaDestinations = KindoraDatabase::query(
        "SELECT destination_id, name, description, image_url, best_season, difficulty_level
         FROM destinations 
         WHERE type = 'antarctica' AND is_active = 1 
         ORDER BY name ASC"
    ) ?: array();

    // Log for debugging
    error_log("Antarctica destinations query result: " . count($antarcticaDestinations) . " records found");
} catch (Exception $e) {
    error_log("Antarctica destinations fetch error: " . $e->getMessage());
}

// Create an associative array for easy lookup by name (if needed)
$destData = array();
foreach ($antarcticaDestinations as $dest) {
...
<html lang="en">
  <head>
    ...
    <title>Antarctica - Kindora</title>
    ...
    <meta name="description" content="Explore the wild beauty of Antarctica with Kindora - Safari adventures and cultural experiences">
  </head>
  <body>
    <br><br><br>
    <!-- Background Video -->
    <div class="video-container">
      <video autoplay muted loop id="bg-video">
        <source src="/Kindora/continents videos/antarctica_intro.webm" type="video/webm" />
      </video>
    </div>

    <!-- Dynamically Loaded Destinations Grid -->
    <div class="card-grid">
      <?php if (!empty($antarcticaDestinations)): ?>
        <?php foreach ($antarcticaDestinations as $destination): ?>
