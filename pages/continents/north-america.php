...
// Fetch all North-america destinations from database
$northamericaDestinations = array();
try {
    $northamericaDestinations = KindoraDatabase::query(
        "SELECT destination_id, name, description, image_url, best_season, difficulty_level
         FROM destinations 
         WHERE type = 'northamerica' AND is_active = 1 
         ORDER BY name ASC"
    ) ?: array();

    // Log for debugging
    error_log("North-america destinations query result: " . count($northamericaDestinations) . " records found");
} catch (Exception $e) {
    error_log("North-america destinations fetch error: " . $e->getMessage());
}

// Create an associative array for easy lookup by name (if needed)
$destData = array();
foreach ($northamericaDestinations as $dest) {
...
<html lang="en">
  <head>
    ...
    <title>North-america - Kindora</title>
    ...
    <meta name="description" content="Explore the wild beauty of North-america with Kindora - Safari adventures and cultural experiences">
  </head>
  <body>
    <br><br><br>
    <!-- Background Video -->
    <div class="video-container">
      <video autoplay muted loop id="bg-video">
        <source src="/Kindora/continents videos/north_america_intro.webm" type="video/webm" />
      </video>
    </div>

    <!-- Dynamically Loaded Destinations Grid -->
    <div class="card-grid">
      <?php if (!empty($northamericaDestinations)): ?>
        <?php foreach ($northamericaDestinations as $destination): ?>
    ...
    <script>
      ...
      function displayItinerary() {
        ...
        let message = `✈️ Your North-american Adventure Plan:\n\n${tripList}\n\n🌟 ${itinerary.length} amazing destinations selected!`;
