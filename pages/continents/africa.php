...
// Fetch all Africa destinations from database
$africaDestinations = array();
try {
    $africaDestinations = KindoraDatabase::query(
        "SELECT destination_id, name, description, image_url, best_season, difficulty_level
         FROM destinations 
         WHERE type = 'africa' AND is_active = 1 
         ORDER BY name ASC"
    ) ?: array();

    // Log for debugging
    error_log("Africa destinations query result: " . count($africaDestinations) . " records found");
} catch (Exception $e) {
    error_log("Africa destinations fetch error: " . $e->getMessage());
}

// Create an associative array for easy lookup by name (if needed)
$destData = array();
foreach ($africaDestinations as $dest) {
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="Explore the wild beauty of Africa with Kindora - Safari adventures and cultural experiences">
  </head>
  <body>
    <br><br><br>
    <!-- Background Video -->
    <div class="video-container">
      <video autoplay muted loop id="bg-video">
        <source src="/Kindora/continents videos/africa_intro.webm" type="video/webm" />
      </video>
    </div>

    <!-- Dynamically Loaded Destinations Grid -->
    <div class="card-grid">
      <?php if (!empty($africaDestinations)): ?>
        <?php foreach ($africaDestinations as $destination): ?>
