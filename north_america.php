...
try {
    $north_america_destinations_result = $conn->query("
        ...
        WHERE d.type = 'north_america' AND d.name != 'My Trip Planner'
        GROUP BY d.destination_id
        ORDER BY d.name
    ");
    $north_america_destinations = $north_america_destinations_result ? $north_america_destinations_result->fetch_all(MYSQLI_ASSOC) : [];
} catch (Exception $e) {
    error_log("Error fetching North America destinations: " . $e->getMessage());
...
<head>
    ...
    <title>North America Destinations - Kindora</title>
    <link href="north_america.css" rel="stylesheet">
    <link rel="icon" type="image/png" href="kindora-logo.ico">
    <link href="common_nav_footer.css" rel="stylesheet">
    <meta name="description" content="Explore North America with Kindora - From vibrant cities to natural wonders">
</head>
<body>
    <!-- Background Video -->
    <div class="video-container">
        <video autoplay muted loop id="bg-video" preload="metadata">
            <source src="continents_videos/north_america_intro.webm" type="video/webm">
            <source src="continents_videos/north_america_intro.mp4" type="video/mp4">
        </video>
        <div class="video-overlay">
            <h1>Discover North America</h1>
            <p>From coast to coast, endless adventures await</p>
        </div>
    </div>
        ...
        <div id="nav2">
            <div class="inspire-wrapper">
                ...
                <div class="scroll-container1" id="inspireScroll">
                    ...
                    <a data-href="north_america.php" class="active"><div class="image-wrapper1">
    ...
    <main>
        ...
        <div class="stats-section">
            <div class="container">
                <div class="stats-grid">
                    <div class="stat-item">
                        <span class="stat-number"><?= count($north_america_destinations) ?></span>
                        <span class="stat-label">Destinations</span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-number"><?= array_sum(array_column($north_america_destinations, 'booking_count')) ?></span>
                        <span class="stat-label">Explorers</span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-number"><?= count($north_america_destinations) > 0 ? number_format(array_sum(array_column($north_america_destinations, 'avg_rating')) / count($north_america_destinations), 1) : '0' ?></span>
        ...
        <div class="container">
            <div class="card-grid">
                <?php if (!empty($north_america_destinations)): ?>
                    <?php foreach ($north_america_destinations as $destination): ?>
                        <div class="video-card destination-card" 
                            ...
                            <div class="card-image">
                                <img src="<?= str_replace('\\\\', '/', htmlspecialchars($destination['image_url'])) ?>" 
                                     ...
                                     loading="lazy"
                                     onerror="this.src='continents/images/north_america_images/default.avif'">
                                     
                                <?php if ($destination['booking_count'] > 8): ?>
                                    <div class="popular-badge">🗽 Top Choice</div>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="card-overlay">
                                ...
                                <?php if ($destination['booking_count'] > 0): ?>
                                    <div class="booking-info">
                                        <span class="booking-count">🇺🇸 <?= $destination['booking_count'] ?> visitors</span>
                                    </div>
                                <?php endif; ?>
                ...
                <?php else: ?>
                    <div class="no-destinations">
                        <h3>North American Adventures Coming Soon</h3>
                        <p>We're curating the best destinations across the USA and Canada.</p>
                    </div>
                <?php endif; ?>
                ...
                <div class="video-card trip-planner-card" onclick="showTripPlanner()">
                    <div class="card-image">
                        <img src="continents/images/north_america_images/trip_planner.avif" alt="My Trip Planner" loading="lazy">
                    </div>
                    <div class="card-overlay">
                        <h3>My America Planner</h3>
                        <p>Plan your coast-to-coast American adventure with our smart planner.</p>
                        <div class="card-actions">
                            <button class="view-btn">📋 Plan Adventure</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
                ...
                .then(data => {
                    document.getElementById('modalContent').innerHTML = `
                        ...
                        <div class="modal-actions" style="margin-top: 20px;">
                            <a href="booking.php?destination=${data.destination_id}" class="book-btn">Explore This Destination</a>
                        </div>
                    `;
        ...
        function showTripPlanner() {
            <?php if (isset($_SESSION['user_id'])): ?>
                window.location.href = 'trip_planner.php?continent=north_america';
            <?php else: ?>
